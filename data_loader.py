...
class Dataset(data.Dataset):
    def _preprocess(self):
        ...
        with open(self.data_dir,encoding='utf-8') as f:
            for line in f.readlines():
                ...
                line_data = {
                    "ent1": {"id": line[0], "name": line[0]},
                    "ent2": {"id": line[1], "name": line[1]},
                    "rel": self.rel2id[line[2]] if line[2] in self.rel2id else self.rel2id['unknown'],
    ...
    def __init__(self, file_name, opt, training=True):
        ...
        if os.path.exists(self.vec_save_dir) and os.path.exists(self.word2id_save_dir):
            self.word2id = json.load(open(self.word2id_save_dir,encoding='utf-8'))
        else:
    ...
    def __getitem__(self, index):
        ...
        if self.training:
            ...
            for i in set(rel):
                rel_mul[i] = 1
            rel = rel_mul
        return word, pos1, pos2, ent1, ent2, mask, length, rel, index

    def init_word(self):
        f = open(self.vec_dir,encoding='utf-8')
        ...
        json.dump(self.word2id, open(self.word2id_save_dir, 'w',encoding='utf-8'))

    def init_rel(self):
        f = open(self.rel_dir,encoding='utf-8')
        self.rel2id = {}
        # 记录每个类别的总数
        self.class_numdict = {}
        f.readline()
    ...
    def get_classnum(self):
        with open(self.data_dir,encoding='utf-8') as f:
            for line in f.readlines():
                line = line.strip().split()
                # 统计各类别在数据中出现的次数
                if line[2] in self.rel2id :
                    self.class_numdict[line[2]] += 1
                else :
                    self.class_numdict['unknown'] +=1
                # 将数据为0的类别改为1
                for i in self.class_numdict:
                    if self.class_numdict[i] == 0:
                        self.class_numdict[i] = 1
...
def collate_fn(X):
    X = list(zip(*X))
    word, pos1, pos2, ent1, ent2, mask, length, rel,index = X
    scope = []
    ind = 0
    for w in word:
    ...
    rel = torch.stack(rel)
    return word, pos1, pos2, ent1, ent2, mask, length, scope, rel,index
